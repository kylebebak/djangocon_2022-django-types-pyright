...
class Thread(BaseModel):
    ...
    moderator = models.ForeignKey(User, related_name="moderated_threads", null=True, on_delete=models.SET_NULL)
    text = models.CharField(max_length=10000, blank=False)
...
def send_notifications():
    ...
    for thread in Thread.objects.prefetch_related("users", "posts"):
        new_posts = list(thread.posts.filter(created_at__gt=timezone.now() - timedelta(days=7))) or None
        if not new_posts:
            continue
